...
async function initDatabase() {
  ...
  if (users[0].values[0][0] === 0) {
    ...
    db.run(`INSERT INTO users (naam, email, wachtwoord_hash, rol, afdeling) VALUES (?, ?, ?, ?, ?)`,
      ['Admin Manager', 'admin@winkel.nl', hash, 'manager', 'Algemeen']
    );
    
    const demoHash = bcrypt.hashSync('demo123', 10);
    db.run(`INSERT INTO users (naam, email, wachtwoord_hash, rol, afdeling) VALUES (?, ?, ?, ?, ?)`,
      ['Jan Jansen', 'jan@winkel.nl', demoHash, 'medewerker', 'Kassa']
    );
    db.run(`INSERT INTO users (naam, email, wachtwoord_hash, rol, afdeling) VALUES (?, ?, ?, ?, ?)`,
      ['Marie de Vries', 'marie@winkel.nl', demoHash, 'medewerker', 'Magazijn']
    );
    db.run(`INSERT INTO users (naam, email, wachtwoord_hash, rol, afdeling) VALUES (?, ?, ?, ?, ?)`,
      ['Pieter Bakker', 'pieter@winkel.nl', demoHash, 'medewerker', 'Vakkenvullen']
    );
    ...
    for (let i = 0; i < 14; i++) {
      ...
      if (dayOfWeek !== 0) {
        db.run(`INSERT INTO shifts (medewerker_id, datum, starttijd, eindtijd, pauze, afdeling, status) VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [2, dateStr, '09:00', '17:00', 30, 'Kassa', 'gepubliceerd']
        );
        
        if (dayOfWeek !== 6) {
          db.run(`INSERT INTO shifts (medewerker_id, datum, starttijd, eindtijd, pauze, afdeling, status) VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [3, dateStr, '12:00', '20:00', 30, 'Magazijn', 'gepubliceerd']
          );
        }
        
        if (dayOfWeek === 3 || dayOfWeek === 5) {
          db.run(`INSERT INTO shifts (medewerker_id, datum, starttijd, eindtijd, pauze, afdeling, status) VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [4, dateStr, '14:00', '22:00', 30, 'Vakkenvullen', 'gepubliceerd']
          );
        }
      }
    }
...
app.put('/api/auth/profile', authenticateToken, (req, res) => {
  try {
    ...
    dbRun('UPDATE users SET naam = ?, telefoon = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [naam, telefoon, req.user.id]);
    const user = dbGet('SELECT * FROM users WHERE id = ?', [req.user.id]);
    delete user.wachtwoord_hash;
    res.json(user);
  } catch (error) {
...
app.post('/api/auth/change-password', authenticateToken, (req, res) => {
  try {
    ...
    dbRun('UPDATE users SET wachtwoord_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [hash, req.user.id]);
...
app.post('/api/users', authenticateToken, requireManager, (req, res) => {
  try {
    ...
    const result = dbRun('INSERT INTO users (naam, email, wachtwoord_hash, rol, afdeling, vakantiesaldo) VALUES (?, ?, ?, ?, ?, ?)',
...
app.get('/api/shifts', authenticateToken, (req, res) => {
  try {
    ...
    let sql = 'SELECT s.*, u.naam as medewerker_naam FROM shifts s JOIN users u ON s.medewerker_id = u.id WHERE 1=1';
    const params = [];
    if (startDatum) { sql += ' AND s.datum >= ?'; params.push(startDatum); }
    if (eindDatum) { sql += ' AND s.datum <= ?'; params.push(eindDatum); }
    if (medewerker_id) { sql += ' AND s.medewerker_id = ?'; params.push(medewerker_id); }
    if (req.user.rol !== 'manager') { sql += ' AND (s.status = ? OR s.medewerker_id = ?)'; params.push('gepubliceerd', req.user.id); }
...
app.post('/api/shifts', authenticateToken, requireManager, (req, res) => {
  try {
    ...
    const result = dbRun('INSERT INTO shifts (medewerker_id, datum, starttijd, eindtijd, pauze, afdeling, status, notities) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
...
app.put('/api/shifts/:id', authenticateToken, requireManager, (req, res) => {
  try {
    const { medewerker_id, datum, starttijd, eindtijd, pauze, afdeling, status, notities } = req.body;
    dbRun('UPDATE shifts SET medewerker_id = ?, datum = ?, starttijd = ?, eindtijd = ?, pauze = ?, afdeling = ?, status = ?, notities = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
...
app.post('/api/shifts/publish', authenticateToken, requireManager, (req, res) => {
  try {
    ...
    dbRun('UPDATE shifts SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE datum >= ? AND datum <= ? AND status = ?', ['gepubliceerd', startDatum, eindDatum, 'concept']);
    const employees = dbAll('SELECT DISTINCT medewerker_id FROM shifts WHERE datum >= ? AND datum <= ?', [startDatum, eindDatum]);
    for (const emp of employees) {
      dbRun('INSERT INTO notifications (ontvanger_id, type, titel, bericht, link) VALUES (?, ?, ?, ?, ?)',
...
app.get('/api/time-registrations', authenticateToken, (req, res) => {
  try {
    const { startDatum, eindDatum, medewerker_id } = req.query;
    let sql = 'SELECT t.*, u.naam as medewerker_naam, s.starttijd as gepland_start, s.eindtijd as gepland_eind FROM time_registrations t JOIN users u ON t.medewerker_id = u.id LEFT JOIN shifts s ON t.shift_id = s.id WHERE 1=1';
    const params = [];
    if (req.user.rol !== 'manager') { sql += ' AND t.medewerker_id = ?'; params.push(req.user.id); }
    else if (medewerker_id) { sql += ' AND t.medewerker_id = ?'; params.push(medewerker_id); }
    if (startDatum) { sql += ' AND t.datum >= ?'; params.push(startDatum); }
    if (eindDatum) { sql += ' AND t.datum <= ?'; params.push(eindDatum); }
...
app.post('/api/time-registrations/checkin', authenticateToken, (req, res) => {
  try {
    ...
    const existing = dbGet('SELECT * FROM time_registrations WHERE medewerker_id = ? AND datum = ? AND uitchecktijd IS NULL', [req.user.id, datum]);
    if (existing) return res.status(400).json({ error: 'Je bent al ingecheckt vandaag' });
    const shift = dbGet('SELECT id FROM shifts WHERE medewerker_id = ? AND datum = ?', [req.user.id, datum]);
    const result = dbRun('INSERT INTO time_registrations (medewerker_id, shift_id, datum, inchecktijd) VALUES (?, ?, ?, ?)', [req.user.id, shift?.id || null, datum, tijd]);
    res.status(201).json(dbGet('SELECT * FROM time_registrations WHERE id = ?', [result.lastID]));
  } catch (error) {
...
app.post('/api/time-registrations/checkout', authenticateToken, (req, res) => {
  try {
    ...
    const tijd = new Date().toTimeString().slice(0, 5);
    const existing = dbGet('SELECT * FROM time_registrations WHERE medewerker_id = ? AND datum = ? AND uitchecktijd IS NULL', [req.user.id, datum]);
    if (!existing) return res.status(400).json({ error: 'Je bent nog niet ingecheckt vandaag' });
    dbRun('UPDATE time_registrations SET uitchecktijd = ? WHERE id = ?', [tijd, existing.id]);
    res.json(dbGet('SELECT * FROM time_registrations WHERE id = ?', [existing.id]));
  } catch (error) {
...
app.post('/api/time-registrations', authenticateToken, (req, res) => {
  try {
    ...
    const shift = dbGet('SELECT id FROM shifts WHERE medewerker_id = ? AND datum = ?', [req.user.id, datum]);
    const result = dbRun('INSERT INTO time_registrations (medewerker_id, shift_id, datum, inchecktijd, uitchecktijd, pauze_minuten, notities) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
app.put('/api/time-registrations/:id', authenticateToken, (req, res) => {
  try {
    ...
    if (req.user.rol !== 'manager' && reg.medewerker_id !== req.user.id) return res.status(403).json({ error: 'Geen toegang' });
    const { inchecktijd, uitchecktijd, pauze_minuten, goedgekeurd, notities } = req.body;
    dbRun('UPDATE time_registrations SET inchecktijd = ?, uitchecktijd = ?, pauze_minuten = ?, goedgekeurd = ?, notities = ? WHERE id = ?',
      [inchecktijd, uitchecktijd, pauze_minuten, goedgekeurd ? 1 : 0, notities, req.params.id]);
    res.json(dbGet('SELECT * FROM time_registrations WHERE id = ?', [req.params.id]));
  } catch (error) {
...
app.get('/api/leave-requests', authenticateToken, (req, res) => {
  try {
    ...
    let sql = 'SELECT l.*, u.naam as medewerker_naam FROM leave_requests l JOIN users u ON l.medewerker_id = u.id WHERE 1=1';
    const params = [];
    if (req.user.rol !== 'manager') { sql += ' AND l.medewerker_id = ?'; params.push(req.user.id); }
    else if (medewerker_id) { sql += ' AND l.medewerker_id = ?'; params.push(medewerker_id); }
...
app.post('/api/leave-requests', authenticateToken, (req, res) => {
  try {
    ...
    if (type === 'vakantie') {
      ...
      if (days > user.vakantiesaldo) return res.status(400).json({ error: 'Onvoldoende vakantiesaldo' });
    }
    const result = dbRun('INSERT INTO leave_requests (medewerker_id, begindatum, einddatum, type, opmerking, aantal_dagen) VALUES (?, ?, ?, ?, ?, ?)',
    ...
    res.status(201).json(dbGet('SELECT * FROM leave_requests WHERE id = ?', [result.lastID]));
  } catch (error) {
...
app.post('/api/leave-requests/:id/process', authenticateToken, requireManager, (req, res) => {
  try {
    ...
    dbRun('UPDATE leave_requests SET status = ?, beoordeling_door = ?, beoordeling_opmerking = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [status, req.user.id, opmerking, req.params.id]);
    if (status === 'goedgekeurd' && request.type === 'vakantie') {
      dbRun('UPDATE users SET vakantiesaldo = vakantiesaldo - ? WHERE id = ?', [request.aantal_dagen, request.medewerker_id]);
    }
    dbRun('INSERT INTO notifications (ontvanger_id, type, titel, bericht, link) VALUES (?, ?, ?, ?, ?)',
      [request.medewerker_id, 'verlof', `Verlofaanvraag ${status}`, `Je verlofaanvraag is ${status}.`, '/vakantie']);
    res.json(dbGet('SELECT * FROM leave_requests WHERE id = ?', [req.params.id]));
  } catch (error) {
...
app.delete('/api/leave-requests/:id', authenticateToken, (req, res) => {
  try {
    const request = dbGet('SELECT * FROM leave_requests WHERE id = ?', [req.params.id]);
    if (req.user.rol !== 'manager' && request.medewerker_id !== req.user.id) return res.status(403).json({ error: 'Geen toegang' });
    if (request.status !== 'in_behandeling') return res.status(400).json({ error: 'Kan alleen aanvragen in behandeling annuleren' });
...
app.get('/api/availability', authenticateToken, (req, res) => {
  try {
    ...
    if (req.user.rol !== 'manager' && medewerker_id != req.user.id) return res.status(403).json({ error: 'Geen toegang' });
    res.json(dbAll('SELECT * FROM availability WHERE medewerker_id = ? ORDER BY dag_van_week', [medewerker_id]));
  } catch (error) {
...
app.post('/api/availability', authenticateToken, (req, res) => {
  try {
    ...
    dbRun('DELETE FROM availability WHERE medewerker_id = ? AND dag_van_week = ? AND type = ?', [req.user.id, dag_van_week, type || 'vast']);
    const result = dbRun('INSERT INTO availability (medewerker_id, dag_van_week, beschikbaar_van, beschikbaar_tot, beschikbaar, type, datum) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [req.user.id, dag_van_week, beschikbaar_van, beschikbaar_tot, beschikbaar ? 1 : 0, type || 'vast', datum]);
    res.status(201).json(dbGet('SELECT * FROM availability WHERE id = ?', [result.lastID]));
  } catch (error) {
...
app.get('/api/shift-swaps', authenticateToken, (req, res) => {
  try {
    let sql = 'SELECT sw.*, a.naam as aanvrager_naam, o.naam as ontvanger_naam, sa.datum as shift_aanvrager_datum, sa.starttijd as shift_aanvrager_start, sa.eindtijd as shift_aanvrager_eind, so.datum as shift_ontvanger_datum, so.starttijd as shift_ontvanger_start, so.eindtijd as shift_ontvanger_eind FROM shift_swaps sw JOIN users a ON sw.aanvrager_id = a.id JOIN users o ON sw.ontvanger_id = o.id JOIN shifts sa ON sw.shift_aanvrager_id = sa.id LEFT JOIN shifts so ON sw.shift_ontvanger_id = so.id WHERE 1=1';
    const params = [];
    if (req.user.rol !== 'manager') { sql += ' AND (sw.aanvrager_id = ? OR sw.ontvanger_id = ?)'; params.push(req.user.id, req.user.id); }
...
app.post('/api/shift-swaps', authenticateToken, (req, res) => {
  try {
    ...
    const result = dbRun('INSERT INTO shift_swaps (aanvrager_id, ontvanger_id, shift_aanvrager_id, opmerking) VALUES (?, ?, ?, ?)', [req.user.id, ontvanger_id, shift_aanvrager_id, opmerking]);
    dbRun('INSERT INTO notifications (ontvanger_id, type, titel, bericht, link) VALUES (?, ?, ?, ?, ?)', [ontvanger_id, 'dienstruil', 'Nieuw ruilverzoek', 'Je hebt een nieuw ruilverzoek ontvangen.', '/dienstruil']);
    res.status(201).json(dbGet('SELECT * FROM shift_swaps WHERE id = ?', [result.lastID]));
  } catch (error) {
...
app.post('/api/shift-swaps/:id/respond', authenticateToken, (req, res) => {
  try {
    ...
    if (swap.ontvanger_id !== req.user.id) return res.status(403).json({ error: 'Geen toegang' });
    ...
    dbRun('INSERT INTO notifications (ontvanger_id, type, titel, bericht, link) VALUES (?, ?, ?, ?, ?)', [swap.aanvrager_id, 'dienstruil', `Ruilverzoek ${status}`, `Je ruilverzoek is ${status}.`, '/dienstruil']);
    res.json(dbGet('SELECT * FROM shift_swaps WHERE id = ?', [req.params.id]));
  } catch (error) {
...
app.post('/api/shift-swaps/:id/approve', authenticateToken, requireManager, (req, res) => {
  try {
    ...
    if (status === 'goedgekeurd') {
      ...
      if (swap.shift_ontvanger_id) dbRun('UPDATE shifts SET medewerker_id = ? WHERE id = ?', [swap.aanvrager_id, swap.shift_ontvanger_id]);
    }
    dbRun('INSERT INTO notifications (ontvanger_id, type, titel, bericht, link) VALUES (?, ?, ?, ?, ?)', [swap.aanvrager_id, 'dienstruil', `Ruil ${status}`, `De dienstwissel is ${status} door een manager.`, '/dienstruil']);
    dbRun('INSERT INTO notifications (ontvanger_id, type, titel, bericht, link) VALUES (?, ?, ?, ?, ?)', [swap.ontvanger_id, 'dienstruil', `Ruil ${status}`, `De dienstwissel is ${status} door een manager.`, '/dienstruil']);
    res.json(dbGet('SELECT * FROM shift_swaps WHERE id = ?', [req.params.id]));
  } catch (error) {
...
app.get('/api/notifications', authenticateToken, (req, res) => {
  try {
    res.json(dbAll('SELECT * FROM notifications WHERE ontvanger_id = ? ORDER BY created_at DESC LIMIT 50', [req.user.id]));
  } catch (error) {
...
app.get('/api/notifications/unread-count', authenticateToken, (req, res) => {
  try {
    const result = dbGet('SELECT COUNT(*) as count FROM notifications WHERE ontvanger_id = ? AND gelezen = 0', [req.user.id]);
...
app.get('/api/reports/hours', authenticateToken, requireManager, (req, res) => {
  try {
    ...
    const report = dbAll(`SELECT u.id, u.naam, u.afdeling, COUNT(s.id) as aantal_diensten,
      SUM((CAST(substr(s.eindtijd, 1, 2) AS REAL) * 60 + CAST(substr(s.eindtijd, 4, 2) AS REAL)) -
          (CAST(substr(s.starttijd, 1, 2) AS REAL) * 60 + CAST(substr(s.starttijd, 4, 2) AS REAL)) -
          COALESCE(s.pauze, 0)) / 60.0 as totaal_uren
      FROM users u LEFT JOIN shifts s ON u.id = s.medewerker_id AND s.datum >= ? AND s.datum <= ? AND s.status = 'gepubliceerd'
      WHERE u.actief = 1 GROUP BY u.id ORDER BY u.naam`, [startDatum, eindDatum]);
    res.json(report);
  } catch (error) {
...
app.get('/api/reports/leave', authenticateToken, requireManager, (req, res) => {
  try {
    ...
    const report = dbAll(`SELECT u.id, u.naam, u.vakantiesaldo,
      ...
      FROM users u LEFT JOIN leave_requests l ON u.id = l.medewerker_id AND l.begindatum >= ? AND l.einddatum <= ? AND l.type = 'vakantie'
      WHERE u.actief = 1 GROUP BY u.id ORDER BY u.naam`, [startDate, endDate]);
    res.json(report);
  } catch (error) {
    res.status(500).json({ error: 'Fout bij genereren rapport' });
  }
...
app.get('/api/dashboard/stats', authenticateToken, (req, res) => {
  try {
    ...
    const stats = {};
    stats.dienstenVandaag = dbGet('SELECT COUNT(*) as count FROM shifts WHERE datum = ? AND status = ?', [today, 'gepubliceerd']).count;
    stats.medewerkersVandaag = dbGet('SELECT COUNT(DISTINCT medewerker_id) as count FROM shifts WHERE datum = ? AND status = ?', [today, 'gepubliceerd']).count;
    stats.openstaandeAanvragen = dbGet('SELECT COUNT(*) as count FROM leave_requests WHERE status = ?', ['in_behandeling']).count;
    if (req.user.rol === 'manager') {
      const hoursResult = dbGet(`SELECT SUM((CAST(substr(eindtijd, 1, 2) AS REAL) * 60 + CAST(substr(eindtijd, 4, 2) AS REAL)) -
        (CAST(substr(starttijd, 1, 2) AS REAL) * 60 + CAST(substr(starttijd, 4, 2) AS REAL)) - COALESCE(pauze, 0)) / 60.0 as total
        FROM shifts WHERE datum >= ? AND datum <= ? AND status = ?`, [weekStartStr, weekEndStr, 'gepubliceerd']);
      stats.urenDezeWeek = hoursResult.total || 0;
    }
    stats.mijnDiensten = dbAll('SELECT * FROM shifts WHERE medewerker_id = ? AND datum >= ? AND datum <= ? ORDER BY datum, starttijd', [req.user.id, weekStartStr, weekEndStr]);
    stats.komendeDiensten = dbAll('SELECT * FROM shifts WHERE medewerker_id = ? AND datum >= ? ORDER BY datum, starttijd LIMIT 5', [req.user.id, today]);
    res.json(stats);
  } catch (error) {
